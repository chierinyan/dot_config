...
function docker-upgrade() {
  docker images | grep 'latest' | awk '{print $1}' | parallel docker pull {}
}
...
function garchive() {
    ...
    pbd=`basename "$pwd"`
    mkdir -p "$pbd"
    find . -mindepth 1 -maxdepth 1 -type d ! -name "$pbd" | parallel -q --jobs 50% zip -0 --junk-sfx --junk-paths --test -r "$pbd"/{}.cbz {}
}
function lsgid() {
    perl -e 'my @gids=(); foreach my $g (split("\n", `find .`)) {if ($g=~/\[(\d+)\](.cbz)?$/) {push @gids, $1}} print join(",", @gids);'
}
function zfill() {
    ls | while read -r f; do mv -i "$f" `printf "%03d" ${f%.*}`.${f##*.}; done
}
function stripdir() {
